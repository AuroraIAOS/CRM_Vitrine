...
| A4 | **[OBSOLETA — errada na premissa; corrigida na Subetapa 03.0, 2026-09-03]** ~~Falta o valor `faltou` no enum de status de `aba_scheduling.agendamentos`.~~ Ele **já existe** como `nao_compareceu` (`db/migrations/009_aba_scheduling.sql:259`), traduzido pela convenção do `CLAUDE.md` §2 — a taxa de falta é calculável desde a Etapa 01. Segue válido só o que faltava de verdade: o valor **`sala_de_espera`** e o KPI que lê o dado. | `db/migrations/` — Subetapa 03.4 | vídeo CT10 |
