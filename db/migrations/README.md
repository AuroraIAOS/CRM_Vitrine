...
| `045_scheduling_schema.sql`, `068`, `075` | `aba_scheduling` | Traduzir nomes; `068` (lembretes) entra no DDL inicial. `075` (profissional exige funcionário ativo): **decisão revista na Subetapa 01.3** — só o desenho de FK (`profissionais.funcionario_id → aba_people.funcionarios(id)`) foi portado; o `CHECK` "ativo exige funcionário" e a RPC de liga/desliga (`scheduling.set_professional`) ficaram de fora porque dependem de `074_employees_born_from_invitation.sql` (fluxo de convite→funcionário que o Vitrine ainda não construiu) — ver `009_aba_scheduling.sql` e `handoffs/instrucoes.md` §5. |
