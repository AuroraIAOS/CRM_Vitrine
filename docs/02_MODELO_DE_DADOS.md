...
- **`aba_treatment.procedimentos_plano.estado`** *(nome real da tabela, fixado pela Subetapa 03.8; este parágrafo dizia `aba_treatment.procedimentos`, escrito antes de a migration existir — e `procedimentos` sozinho é a tabela do CATÁLOGO, que é outra coisa)*: `proposto → planejado → em_execucao → executado`, com `nao_mais_necessario` como saída lateral a partir de `planejado` ou `em_execucao`. **Três invariantes que valem mais que a lista:** (1) **só `proposto` se apaga** — depois disso o procedimento nunca mais é excluído, só some do plano e **permanece na lista geral**, porque o histórico do que se propôs é o que protege a clínica; (2) **consentir move `proposto → planejado` automaticamente**, para que não se apague tratamento que o paciente já consentiu; (3) **`executado` é fato afirmado, com data e autor — nunca derivado**. Este terceiro ponto substitui a decisão da Subetapa 03.7, que fazia `executado` nascer da comparação entre duas evoluções: a trava de finalização de contrato da 03.8.a se apoia nele, e trava financeira não se apoia em inferência.
...
>
> 1. **O nome.** Schema **`aba_treatment`**, chave de módulo `treatment`, label "Planos de tratamento" (decisão D-I2 de Max, 2026-09-03). **"Orçamento" continua sendo a palavra da interface.** **[Rótulo revisto para "Planos" pela decisão D-V1, 2026-09-04, e é esse que a migration `045` gravou em `access.modules`. **[E revisto de novo no fechamento da 03.8, também por ordem de Max: fica **"Plano"**, no singular, para padronizar com o vocabulário — migration `047`.]** `CLAUDE.md` §2 registra as duas revisões.]**
...
> **✅ CONSTRUÍDO na Subetapa 03.8.a, 2026-09-05** — migration `048_orcamento_preco_resolvido.sql`. Esta nota deixa de ser desenho e passa a descrever o que existe. **Os nomes reais:** `aba_finance.tabelas_preco` (a entidade com vigência), `tarifas` (a linha de preço), `orcamentos` (a vista financeira de UMA opção do plano) e `itens_orcamento` (a linha do orçamento), mais `aba_scheduling.tipos_profissional`, que o degrau 2 exigia.
>
> - **A escada é `aba_finance.resolver_preco(procedimento, cliente, profissional, data)`** e devolve `valor`, `tabela_preco_id`, `degrau` e o número do degrau. **Ela não tem parâmetro de tabela de preço**, e é essa ausência que cumpre "sem escolha na tela": não existe caminho por onde a interface passe uma tabela. A verificação (g) da migration lê a assinatura no catálogo e recusa a migration se alguém acrescentar esse parâmetro. Último recurso é `aba_catalog.procedimentos.preco_base`, com o degrau devolvido como `catalogo` — sem esse fundo, toda conta nasceria com orçamento vazio no dia da implantação.
> - **Três estados de vigência:** `rascunho` (editável) → `comprometida` (com data, autor e **tarifa imutável**) → `encerrada` (continua existindo, porque é a proveniência de todo valor congelado). Reajuste é `reajustar_tabela_preco()`, que **cria tabela nova** copiando as tarifas com o percentual; `encerrar_tabela_preco()` tira uma tabela da escada sem substituí-la. Gatilhos recusam alterar **e apagar** tarifa comprometida, e recusam mover o degrau ou a vigência de uma tabela comprometida — mover o degrau reescreveria o passado sem tocar em nenhum valor.
> - **O degrau `Tipo de profissional` exigiu catálogo próprio.** `aba_scheduling.profissionais.especialidade` é TEXT livre, sem tela e sem catálogo, e nenhuma chave estrangeira se pendura nisso. Nasceu `aba_scheduling.tipos_profissional` (por conta, com dois tipos semeados) e `profissionais.tipo_profissional_id`. `especialidade` fica: "Endodontia" é especialidade, "Especialista" é tipo, e é o tipo que move o preço.
> - **Os degraus `Clínica` e `Grupo` nascem sem discriminador**, de propósito: multiunidade é a **Subetapa 03.9** e não existe hoje tabela de unidade para uma chave apontar. Eles já distinguem pela **precedência**, e a 03.9 acrescenta a coluna por adição, sem reescrever a escada.
> - **A DECISÃO DE MODELAGEM QUE O P-SUB PRODUZIU, e ela é a mais importante: `itens_orcamento` NÃO guarda `dente` nem `faces`.** A `047` acabara de revogar essas colunas em `aba_treatment`; copiá-las para `aba_finance` daria a qualquer papel com `finance.read` leitura de dado de saúde **sem alcance clínico e sem rastro** — a fronteira não seria rompida, seria contornada por um caminho novo. O item aponta para `procedimento_plano_id`, e o detalhe sai por **`aba_finance.ler_orcamentos(plano_id)`**, que decide pelo alcance do chamador: com alcance devolve dente e face e **registra em `aba_health.log_acesso`**; sem alcance devolve o mesmo orçamento, com os mesmos valores, sem dente, sem face e **sem log**. O retorno traz `com_detalhe_clinico` para a tela **dizer** que está incompleta. **Coluna que não existe é proteção mais forte que coluna revogada.**
...
> **✅ CONSTRUÍDO na Subetapa 03.8, 2026-09-04** — migrations `045_aba_treatment.sql` e `046_expose_aba_treatment_schema.sql`. Esta seção deixa de ser desenho e passa a descrever o que existe. **Os nomes reais:** `aba_treatment.fases` (a linha), `opcoes` (a coluna), `diagnosticos` (filhos do PLANO, para atravessarem as colunas) e `procedimentos_plano` (a célula). A porta é `aba_treatment.pode_planejar(cliente_id, acao)` — `aba_health.pode_acessar()` **E** `access.can('treatment', ação)`. **A fila de trabalho ficou DERIVADA** (diagnóstico sem procedimento, um `NOT EXISTS`) em vez de ganhar coluna: coluna a mais ali seria a mesma informação com dois donos. **Sem preço, sem contrato, sem fatura** — 03.8.a.
...
> **✅ EXECUTADO na Subetapa 03.7.a, 2026-09-04, sem migration nenhuma.** O item de `marcacoes` passou a ser o registro do dente: `faces` carrega as faces do **trabalho** — em vocabulário português, o de `db/migrations/043` (`mesial`/`distal`/`vestibular`/`lingual`/`oclusal`/`incisal`) — e as faces do **achado** vivem em `achados[].faces`, lista separada. O item sentinela `estado_nativo` da 03.7 **deixou de existir**: a projeção legível virou o dado inteiro. **É de `faces` que a 03.8 monta a linha do plano**, e a suíte de RLS tem caso permanente afirmando que as duas listas são diferentes (`05_aba_health.spec.ts`, "A FACE DO TRABALHO NÃO É A FACE DO ACHADO"). Junto vieram `denticao` por posição (§12 A3) e `trabalhos[]` com os cinco estados, sendo `executado` **fato afirmado com `executadoEm` e `executadoPor`** — que é o par de campos que a trava de finalização de contrato da 03.8.a vai ler.
...
**"Plano" e "orçamento" também são coisas distintas, e as duas existem na tela** (Max, 2026-09-04): **plano** é o planejamento clínico — o conjunto de procedimentos a executar naquele paciente; **orçamento** é a vista financeira dele — o preço a pagar pela execução do que foi contratado. A recepção e o profissional veem os dois; **só o profissional autor edita o plano**, e **só `admin` mexe em parcela, desconto, juros, mora e promoção** (§13.3) — **construído na Subetapa 03.8.a** como trava de COLUNA em gatilho (`aba_finance.exigir_alcada_financeira()`), alcançando também `contratos` e `parcelas_contrato`, que já existiam: policy decide quais LINHAS, e a pergunta aqui é quais COLUNAS.
...
> **✅ CONSTRUÍDO na Subetapa 03.8.b, 2026-09-14 — migration `052_contrato_assinatura_trava_dupla.sql`.** `aba_finance.itens_contrato` tem o arco de três braços, com o gatilho de paciente. A cópia da opção aceita segue a **D-F9**: uma linha `plano_id` com a soma e uma linha por pacote. **O cardápio é `aba_finance.ofertas`, e não `aba_catalog.ofertas` (D-F13):** a view lê `aba_treatment.planos`, e no catálogo inverteria a dependência entre módulos. Documento canônico e hash em `contratos`; assinaturas em `assinaturas_contrato`, cada uma sobre o hash que assinou; a dispensa da D-V8 em `aba_catalog.dispensas_contrato`, por procedimento (D-F11). A cobertura de execução por face mora em `aba_treatment.execucoes_face` (D-F10), com o regime clínico completo; a do pacote é o saldo de sessões, e a do avulso é `execucoes_item_contrato` (D-F12). A trava dupla é a guarda de estado de `contratos`. Nenhuma coluna clínica em `aba_finance`.
...
- **[D-F8, Max, 2026-09-14] Nada desta seção foi construído na 03.8.b.** O contrato grava só quem responde por ele (`contratos.profissional_id`, o profissional que aprovou o orçamento). A trava por autor, o coautor e a sucessão ao `owner` são subetapa própria, depois da 03.9 e da **03.14**: o coautor depende do regime de referência e contrarreferência, que ainda não existe.
...
> **⚠️ NOTA DE REVISÃO (2026-09-06) — o gancho 1 abaixo está no degrau ERRADO, e corrigir agora é barato.** Ao encaixar casos reais nos cinco degraus da escada da 03.8.a, Max expôs um buraco que este parágrafo não previa: **não existe preço por GRUPO de pacientes**. "Todas as mães em maio", "todos os servidores da prefeitura", "todos os pacientes de uma apólice" — hoje só existe o degrau **Paciente**, que é individual, e uma cortesia para 40 pessoas exigiria 40 tabelas. O gancho 1 reserva justamente esse degrau individual para receber o convênio; mas **convênio é preço de grupo por definição** — é a apólice que negocia, não cada paciente. Amarrá-lo ao degrau individual reproduziria o problema com 40, 400 ou 4.000 linhas. **O texto abaixo não se apaga** (`CLAUDE.md` §8 e §10): fica como está, com esta ressalva ao lado. **DECIDIDO em 2026-09-10 (D-F5): o degrau de grupo entra na Etapa 03, como Subetapa 03.8.d** — na posição 2 da escada, entre Paciente e Tipo de profissional. O gancho do convênio passa a ser esse degrau, e não o individual. **O convênio em si continua fora do MVP** por D-V5: nenhuma operadora, apólice, carência ou cobertura se constrói — o que existe é o lugar onde elas vão encaixar. Fonte: `docs/08_CAMINHO_FELIZ.md` §5.
