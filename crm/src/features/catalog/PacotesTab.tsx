...
function FormularioNovoPlano({ onCriado, onCancelar }: { onCriado: (pacoteId: string) => void; onCancelar: () => void }) {
  ...
  return (
    <Card className="flex flex-col gap-2.5 p-4">
      <span className="text-[12.5px] font-medium text-foreground">Novo pacote</span>
      <form
        ...
        onSubmit={(e) => {
          ...
          criar.mutate(
            ...
            { onSuccess: (pacoteId) => onCriado(pacoteId) },
          );
        }}
      >
        <div className="flex flex-1 flex-col gap-1">
          <label className="text-[11px] font-medium text-secondary-foreground">Nome do pacote</label>
          <input
            required
        ...
        <Button type="submit" size="sm" disabled={criar.isPending}>
          {criar.isPending ? "Criando..." : "Criar pacote"}
        </Button>
...
function FormularioNovoItem({ pacoteId }: { pacoteId: string }) {
  ...
  const procedimento = (procedimentos ?? []).find((s) => s.id === procedimentoId);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        criar.mutate(
          { pacoteId, procedimentoId, varianteId: varianteId || undefined, sessoesIncluidas: sessoes },
    ...
    >
      ...
      <select
        ...
        disabled={!procedimento || procedimento.variantes.length === 0}
      >
  ...
  return (
    <Card className="flex flex-col gap-3 p-4">
      ...
      <div className="flex flex-col gap-1.5">
        {plano.itens.length === 0 && <span className="text-[11.5px] text-muted-foreground">Nenhum item ainda — adicione ao menos um procedimento.</span>}
        {plano.itens.map((i) => (
      ...
      <FormularioNovoItem pacoteId={plano.id} />
    </Card>
  );
}
    ...
    <div className="flex flex-col gap-3">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => setMostrarNovo((v) => !v)}>
          {mostrarNovo ? "Cancelar" : "+ Novo pacote"}
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        {lista.length === 0 && (
          <Card className="p-6 text-center text-[11.5px] text-muted-foreground md:col-span-2">Nenhum pacote cadastrado ainda.</Card>
        )}
      </div>
